...
C Lehrstuhl für polymerbasierter Leichtbau
C
C CHANGELOG / REVISIONS (Updated 2026):
C   1. Bugfix (Puck FF): Fixed sign error for compressive fiber fracture, 
C      ensuring e(1) is positive for correct failure triggering.
C   2. Bugfix (Puck IFF): Corrected mathematical formula for Pnt to 
C      -1/tan(2*THETAF) and Rnt to Yc/(2*tan(THETAF)) to guarantee 
C      self-consistency under pure transverse compression.
C   3. Opt 1 (Speed): Replaced 181-step linear angle search for Puck IFF
C      with a Two-Pass method (5-deg coarse + 1-deg fine). Reduced 
C      costly evaluations from 181 to 48 per integration point.
C   4. Opt 2 (Speed): Deferred Jacobian Update. Moved the second ortho3D 
C      call inside the active damage block (dmg < 1.0 or e > 1.0) to skip 
C      expensive stiffness matrix recalculations for purely elastic elements.
C   5. Feature (Outputs): Restructured State Variable (SDV) mapping. Failure 
C      indices (e) now map to SDV7-12, fracture angles to SDV14-19, and 
C      failure flags (fflags) moved to SDV20-25.
C   6. Feature (IFF Modes): Enhanced Puck IFF logic to explicitly separate 
C      Mode A (e3), Mode B (e4), and Mode C (e5) and record their respective 
C      fracture angles, rather than just the overall maximum.
C   7. Fix (Stability): Introduced residual stiffness lower bounds (10% for 
C      fiber, 5% for matrix/shear) to prevent singular stiffness matrices.
C   8. Fix (Trigger): Changed extreme failure trigger from maxval(e) > 2.0 
C      to specifically e(1) > 2.0 (fiber failure only).
C	
C	WRITTEN BY Yang Liu 
C   DATE: 25.06.2026
C*************************************************************************
...
!     name incorrectly, the compiler won't catch your typo.	  
     ...
     4 JSTEP(4),UPSTRAN(NTENS),dmg(6),e(6),angles_out(6)
     
	  INTEGER fflags(6), Del_Stat,failure_id,damage_id
      DOUBLE PRECISION MGF,le,let,THETAMAX_OUT
...
!#----------------------------------------------------------#             
 ...
 ! includes: damage factors, failure flags, and element deletion status  
    ...
    fflags(1:6)=STATEV(20:25)

    angles_out(1:6)=STATEV(14:19)
...
!#----------------------------------------------------------# 
    ...
    STATEV(1:6)=dmg(1:6)  
    STATEV(7:12)=e(1:6)
    STATEV(14:19)=angles_out(1:6)
    STATEV(20:25)=fflags(1:6)	
    STATEV(13)=Del_Stat
...
! call failure_calc subroutine to calculate failure indicies
    ...
    &EpsXt,EpsXc,EpsYt,EpsYc,EpsZt,EpsZc,GamS12,GamS13,GamS23,THETAF,MGF,ANU12,ANU12f,E11,E11f,angles_out)
...
! or previous damage presist      
   ...
   if ((maxval(e).GT.1.0) .OR. (minval(dmg).LT.1.0)) then
    ...
    do I=1,6 
        ! +++ New code: direction-specific degradation lower bounds
        ! dmg is stiffness retention factor: dmg=1.0 (undamaged) ~ dmg=0.0 (fully failed)
        ! Fiber direction (i=1): retain at least 10% stiffness for numerical stability
        ! Matrix normal & shear dirs (i=2~6): allow up to 99% degradation
        !   so that IFF stress redistributes properly to fibers
        if (I .EQ. 1) then
            if (dmg(I) .LT. 0.10d+0) then
                dmg(I) = 0.10d+0   ! fiber: min 10% stiffness retained
            end if
        else
            if (dmg(I) .LT. 0.05d+0) then
                dmg(I) = 0.05d+0   ! matrix/shear: min 5% stiffness retained
            end if
        end if
    ...
    ! Opt 2: re-compute stresses with updated dmg only when damage is active
    !        undamaged integration points skip this call (stresses from L299 remain valid)
    call ortho3D (E11,E22,E33,G12,G13,G23,ANU12,ANU13,ANU23,dmg,STRESS,UPSTRAN,DDSDDE,damage_id)

    end if
    ! If no damage: stresses from first ortho3D call (before failure_calc) remain valid
...
! Fiber fracture Tensile/Compressive
    ...
    if(A.GE.0.0) then
        ...
        e(1)=(1.0d+0/Xt)*A        ! FF tensile index (A > 0)
                 
    else if(A.LT.0.0) then 
        ! Fix: must be positive so compression failure can be detected (e(1) > 1.0)
        e(1)=(-1.0d+0/Xc)*A       ! FF compressive index (A < 0 -> e(1) > 0)
              
    end if
             ! Inter-Fiber fracture 

    ! Puck IFF strength & slope parameters - computed ONCE outside the loop (constants)
    ! Rn:  fracture resistance for normal tensile stress (Mode A)
    ! Rn1: fracture resistance for longitudinal shear
    ! Pnt: slope parameter for normal/transverse shear coupling
    ! Rnt: transverse shear fracture resistance - derived to ensure self-consistency:
    !      under pure compression -Yc, IFF = 1.0 exactly at theta = THETAF
    ! Pn1: slope parameter for normal/longitudinal shear coupling
    Rn  = Yt
    Rn1 = S12
    ! Fix: Pnt = -1/tan(2*THETAF), NOT -1/(2*tan(2*THETAF))
    !   Derivation: for IFF to be maximum at theta=THETAF and equal 1.0 at sigma_2=-Yc,
    !   both conditions together require: Pnt=-1/tan(2*THETAF), Rnt=Yc/(2*tan(THETAF))
    Pnt = -1.0d+0/tan(2.0d+0*THETAF)
    Rnt = Yc/(2.0d+0*tan(THETAF))   ! self-consistent with the corrected Pnt
    Pn1 = Pnt*(Rn1/Rnt)

    ! Step-wise search for failure plane 
    e(2) = 0.0d0                        
    e(3) = 0.0d0 ! Modus A: Matrix Tension
    e(4) = 0.0d0 ! Modus B: Matrix Shear (theta = 0)
    e(5) = 0.0d0 ! Modus C: Matrix Compression (theta != 0)
    e(6) = 0.0d0 ! Not used in Puck, initialize to avoid garbage output in SDV12
    THETAMAX = 0.0d0
    THETA_A = 0.0d0
    THETA_B = 0.0d0
    THETA_C = 0.0d0

    ! Opt 1: Two-pass fracture plane search (37 + ~11 evals instead of 181)
    ! Pass 1: coarse search at 5-degree intervals
    J_COARSE = 0
    do J = 0, 180, 5
        THETA = -Pi/2.0d+0 + J*(Pi/180.0d+0)
        SFP =(Stress(2)*cos(THETA)**2)&
        &+(Stress(3)*sin(THETA)**2)+(2.0d+0*Stress(6)*sin(THETA)*cos(THETA))
    ...
    ! Pass 2: fine search (1-degree) in +-5 degree window around coarse maximum
    ...
    if (failure_id .eq. 7) then
        angles_out(2) = THETAMAX * 180.0d0 / Pi       ! SDV15: overall max IFF angle
        angles_out(3) = THETA_A   * 180.0d0 / Pi       ! SDV16: Mode A fracture angle

        ! SDV17: Mode B fracture angle; -999 = Mode B not triggered this increment
        if(e(4).GT.0.0d0) then
            angles_out(4) = THETA_B * 180.0d0 / Pi
        else
            angles_out(4) = -999.0d0
        end if

        ! SDV18: Mode C fracture angle; -999 = Mode C not triggered this increment
        if(e(5).GT.0.0d0) then
            angles_out(5) = THETA_C * 180.0d0 / Pi
        else
            angles_out(5) = -999.0d0
        end if
    end if
